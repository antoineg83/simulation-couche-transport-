...
unsigned short checksum_client_transport(message *mes,int d ) //fonction pour calculer le checksum client 
{
    ...
    somme += (unsigned short)mes->nb_port_desti; //on fait la retenue après chaque addition
...
void ecrire_mem_partagee_fragments(for_serv *zone, unsigned int *donnee, int taille, int semid_serv) {
    int fragment_taille= 10000; // taille d'un fragment
    int debut = 0;
    ...
    while (debut < taille) 
	{
        ...
        if (debut + fragment_taille > taille) 
		{
            count = taille - debut; // dernier fragment plus petit
        } 
		else 
		{
            count = fragment_taille;      // fragment complet
        }

        // Copie le fragment dans la mémoire partagée
        for (int i = 0; i < count; i++) 
		{
            zone->donnee[debut+i] = donnee[debut + i]; // debut +i pour pas ecraser les fragments précedents 
        }
        debut += count;
    }
}
void transport (int n,int m,int tube_mess_transp[2],short port[],int tube_verif[][2],int d)

{
	...
	while (read(tube_mess_transp[0],&m_lue.taille,sizeof(int)) > 0)
	{
		...
		else 
		{
			...
			int shmid = shmget(cle, sizeof(for_serv) + d * sizeof(unsigned int),IPC_CREAT | 0666);
			if (shmid == -1) 
			{
   				perror("Erreur shmget");
    			exit(1);
			}
            key_t cle_serv = ftok("cle_serv", serveur_id + m + 1); //recuperation  de la clé du sémaphore 
            int semid_serv = semget(cle_serv, 1, 0666);
			...
			if (verif==0)
			{
                ...
                ptr_vers_zone->taille = m_lue.taille;
                ptr_vers_zone->num_mess = m_lue.nb_mess;
				ecrire_mem_partagee_fragments(ptr_vers_zone, m_lue.donnee,m_lue.taille,semid_serv);
                ptr_vers_zone->checksum = ~checksum_transport_serveur(ptr_vers_zone,m_lue.taille);
                 // Signale au serveur qu'il y a un fragment prêt
                struct sembuf V;
                V.sem_num = 0;
                V.sem_op  = 1;
                V.sem_flg = 0;
                semop(semid_serv, &V, 1); // previent quand il y a un fragment pour éviter qu'un fragment écrase le précédent 
...
for_serv* init_memoire_serveur(int s,char *path,int d)   //cree la memoire partager de chaque serveur 
{
    key_t cle_serv = ftok(path, s + 1);
    int shmid = shmget(cle_serv, sizeof(for_serv) + d * sizeof(unsigned int), IPC_CREAT | 0666);
    if (shmid == -1) //test d'erreur 
	{
		perror("shmget");
		 exit(1); 
    ...
    memoire_serveur->donnee = (unsigned int*)(memoire_serveur + 1);
    return memoire_serveur;
}

void serveur(int s, int semid, struct sembuf P, struct sembuf V, int n, short *port, int m,int d)
{
	...
	for_serv * memoire_serveur = init_memoire_serveur(s,"cle_serv",d);
...
int main(int argc, char* argv[])
{
	...
	{
		transport (n,m,tube_mess_transp,port,tube_verif,d); 
	}
	...
	for( int s = 0; s<m;s++)//creation des m serveurs
	{
		...
		{
			serveur(s,semid, P, V,n,port,m,d);
		}
	}
	...
	// Attente de tous les processus pour éviter les zombies
    for (int i = 0; i < n + 1 + m; i++)
	 {
        wait(NULL);
    }

    // --- Suppression de la mémoire partagée des ports ---
    shmdt(port);
    shmctl(shmid, IPC_RMID, NULL);

    // --- Suppression de la mémoire partagée de chaque serveur ---
    for (int s = 0; s < m; s++) 
	{
        key_t cle_serv = ftok("cle_serv", s + 1);
        int shmid_serv = shmget(cle_serv, sizeof(for_serv) + d * sizeof(unsigned int), 0666);
        if (shmid_serv != -1) 
		{
    ...
    // --- Suppression des sémaphores principaux ---
    semctl(semid, 0, IPC_RMID);

    // --- Suppression des sémaphores de chaque serveur ---
    for (int s = 0; s < m; s++) 
    {
        key_t cle_sem_serv = ftok("cle_serv", s + m + 1);
        int semid_serv = semget(cle_sem_serv, 1, 0666);
