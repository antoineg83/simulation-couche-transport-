...
{
	unsigned int taille;//taille données
	unsigned short num_port_desti;//numero port destination
...
{
    int taille_fragment = 10000;//arbitraire mais permet de ne pas surcharger
    int debut = 0;
    int nb = 0;//nb de fragmentation faites
    while(debut < taille)
	{
        ...
        else //sinon count sert d'accumulateur temporaire du nb d'entiers pris en fragment
		{ 
        
            count = taille_fragment;//donc count = 10000
        }
        ...
        if (nb%50 == 0)
        {
            printf("[ATT] fragment numero %d envoyer pour client->transport\n",nb);
            fflush(stdout);
        }
...
void client (int i,int n,int d,int tube_verif[][2],int tube_mess_transp[2],int semid_client)
{
	...
	{  
        ...
        do
		{
	   		semop(semid_client,&P,1);//on accede en mutex au tube commun
           	write(tube_mess_transp[1], &mes.taille, sizeof(unsigned int));
...
void ecrire_donnee_tube_trans_serv_fragment(message* m_lue,short serveur_id,int tube_trans_serv[][2])
{

	int taille_fragment= 10000; // taille d'un fragment
	int debut = 0;
	int nb = 0;
	while(debut < m_lue->taille)
	{
		int count; //nb d'entier par fragment
		if (debut+taille_fragment > m_lue->taille)
		{
	
			count = m_lue->taille - debut; // si il reste moins d'entier que taille_fragment il prend ce qu'il reste pour l'ajouter dans le write 
		}
		else
		{ 
		...
		while (ecrit < nb_octets)
		{
			...
			int val = write(tube_trans_serv[serveur_id][1],((char*)m_lue->donnee) + debut*sizeof(unsigned int) + ecrit, nb_octets - ecrit ); //ecris une partie du fragment dans le pipe et renvoie le nombre d'octets écris 
			if (val <= 0)
			{ 
				perror("write");
				exit(1);
		...
		if (nb%50 == 0)
		{
			...
			printf("[ATT] fragment numero %d envoyer pour transport->serveur\n",nb);
			fflush(stdout);
		}
...
//on envoie d'abord la taille 
	...
	write(tube_trans_serv[serveur_id][1],&m_lue->taille,sizeof(unsigned int));
	...
	ecrire_donnee_tube_trans_serv_fragment( m_lue,serveur_id,tube_trans_serv);
...
void lire_message_fragments(int tube_mess_transp, unsigned int *donnee, int taille)
{
    ...
    while(debut < taille)
    {
        ...
        if (debut+taille_fragment > taille)
        {
            count = taille - debut; // si il reste moins d'entier que taille_fragment il prend ce qu'il reste pour lire avec le read 
        }
        else 
        {
            count = taille_fragment;
...
void lire_message_fragments_serveur(int tube_trans_serv[][2], unsigned int *donnee, int taille,int s)
{
    int taille_fragment = 10000;
    int debut = 0;
    int nb = 0;
    while(debut < taille)
    {
        ...
        while (lus < nb_octets)
        {
            int val = read(tube_trans_serv[s][0],((char*)donnee + debut*sizeof(unsigned int)) + lus, nb_octets - lus ); //ecris une partie du fragment dans le pipe et renvoie le nombre d'octets écris
...
void transport (int n,int m,int tube_mess_transp[2],short port[],int tube_verif[][2],int tube_trans_serv[][2]){
	...
	while(read(tube_mess_transp[0],&m_lue.taille,sizeof(unsigned int))>0)// continue jusqu'a ce qu'il ny est plus aucun message dans le tube
	{ 
		...
		write(tube_verif[m_lue.num_port_desti][1],&verif,sizeof(unsigned int));
		printf("Transport envoie verif %d au client %d\n", verif, m_lue.num_port_desti);
...
void serveur(int s, int semid, struct sembuf P, struct sembuf V, int n, short *port, int m,int d,int tube_trans_serv[][2])
{
	...
	while (read(tube_trans_serv[s][0],&serv.taille,sizeof(unsigned int))>0)
	{
		serv.donnee = malloc(serv.taille * sizeof(unsigned int));
		lire_message_fragments_serveur(tube_trans_serv,serv.donnee,serv.taille,s);
		...
		if((verif = verif_checksum_serveur(&serv,serv.taille)) == 0)
		{
			total_valides++;
		}
		else
...
int main(int argc,char* argv[]){
	...
	if (f1 == NULL)
	{
        perror("fichier (protec_w)  non créé/ouvert");
        exit(1);
    }
   	fclose(f1);
	...
	//-----Creation de la mémoire partagée pour l'écoute des ports par les serveurs + (init fichier pour clé sémaphores : ipc_key)------

	FILE *f2; //crée les fichiers utilisés dans ftock pour la mémoire partagé des serveurs                                                                   
	f2 = fopen("mem_serv", "a");// on ouvre en appending de façon arbitraire juste pour créer le fichier si il n'existe pas, il ne servira que de clépour ftock                                                                         
	if (f2 == NULL)
	{
	...
	//pour ipc_key
	 ...
	 FILE *f3; //crée fichier utilisés dans ftock pour le semaphore d'accès à la mémoire partagé
        f3 = fopen("ipc_key", "a");// on ouvre en appending de façon arbitraire juste pour créer le fichier si il n'existe pas, il ne servira que de clépour ftock
        if (f3 == NULL)
		{
	...
	semctl(semid_client,0,IPC_RMID);
	exit(0);
